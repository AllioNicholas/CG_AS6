	...
	m_shadowEps(0.0f)
{
	...
	m_commonCtrl.addToggle((S32*)&m_renderMode, 0,							FW_KEY_NONE,	"Render normal" );
	...
	m_commonCtrl.addToggle(&m_useDiffuseTexture,							FW_KEY_NONE,	"Use diffuse texture" );
	...
	m_lights[0].first.setPosition(Vec3f(1, 0, 1));
	Mat3f o;
	...
	// YOUR SHADOWS HERE: just add more...
	// Hint: look at CameraControls::getOrientation() and copy-paste it
	// to build the orientation matrix for the lights, for example.
	...
	for (auto& lpair: m_lights)
		lpair.second.setup(Vec2i(1024, 1024));
...
void App::renderFrame(GLContext* gl)
{
	...
	if (m_shadowMode) {
		for (auto& lpair: m_lights)
			lpair.first.renderShadowMap(gl, m_mesh, &lpair.second);
...
void App::renderWithNormalMap(GLContext* gl, Mat4f worldToCamera, Mat4f projection)
{
	...
	// YOUR SHADOWS HERE: Add your lights here, and remember to set their intensities
	// (the hard-coded lights are removed)
	Vec3f lightDir[] = {
		m_lights[0].first.getPosition(), Vec3f( -1, -1, 0 )
	};
	Vec3f I[] = {
		Vec3f(.8, .8, .8), Vec3f( 0.3, 0.2, 0.2 )
	};
	int nlights = FW_ARRAY_SIZE( lightDir );
	// R5: In what space is lightDir before and after this loop?
	for ( int i = 0; i < nlights; ++i )
	{
		lightDir[i] = (worldToCamera * Vec4f( lightDir[i].normalized(), 0.0f )).getXYZ();
	}

	gl->setUniformArray(m_shaderProgram->getUniformLoc("lightDirections"), nlights, lightDir );
	gl->setUniformArray(m_shaderProgram->getUniformLoc("lightIntensities"), nlights, I );
	gl->setUniform(m_shaderProgram->getUniformLoc("numLights"), FW_ARRAY_SIZE(lightDir) );
	...
	for (int i = 0; i < m_lights.size(); i++) {
		...
		// Yes, each array item is also a single uniform in opengl
	...
	for (int i = 0; i < m_mesh->numSubmeshes(); i++)
    {
        ...
        gl->setUniform(m_shaderProgram->getUniformLoc("specularUniform"), mat.specular * 0.5f);
        gl->setUniform(m_shaderProgram->getUniformLoc("glossiness"), mat.glossiness);

        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, mat.textures[MeshBase::TextureType_Diffuse].getGLTexture());
        gl->setUniform(m_shaderProgram->getUniformLoc("useDiffuseTexture"), mat.textures[MeshBase::TextureType_Diffuse].exists() && m_useDiffuseTexture);
